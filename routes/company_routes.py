...
@company_bp.route('/api/company/<int:company_id>/follow', methods=['POST'])
def api_toggle_follow_company(company_id):
    ...
    try:
        ...
        existing = db.query(CompanyFollow).filter_by(user_id=user_id, company_id=company_id).first()
        if existing:
            db.delete(existing)
            db.commit()
            count = db.query(CompanyFollow).filter_by(company_id=company_id).count()
            return jsonify({'success': True, 'following': False, 'follower_count': count})
        else:
            new_f = CompanyFollow(user_id=user_id, company_id=company_id)
            db.add(new_f)
            # Notify company
            db.add(Notification(
                user_id=comp.user_id,
                title="New Follower",
                message=f"{session.get('name', 'A student')} started following {comp.company_name}.",
                category="Follow"
            ))
            db.commit()
            count = db.query(CompanyFollow).filter_by(company_id=company_id).count()
            return jsonify({'success': True, 'following': True, 'follower_count': count})
...
@company_bp.route('/api/company/<int:company_id>/is-following')
def api_is_following_company(company_id):
    user_id = session.get('user_id')
    db = SessionLocal()
    try:
        count = db.query(CompanyFollow).filter_by(company_id=company_id).count()
...
@company_bp.route('/api/companies/search')
def api_search_companies():
    ...
    try:
        ...
        for c in companies:
            c_name = (c.company_name or '').lower()
            c_ind = (c.industry_type or '').lower()
            c_loc = (c.location or '').lower()

            # Required skills from all open jobs
            skills = set()
            for j in c.jobs:
                if j.status == 'Open':
                    for jrs in j.required_skills:
                        if jrs.skill:
                            skills.add(jrs.skill.name)
            for i in c.internships:
                if i.status == 'Open':
                    for jrs in i.required_skills:
                        if jrs.skill:
                            skills.add(jrs.skill.name)

            skills_str = ' '.join(skills).lower()

            if not query or query in c_name or query in c_ind or query in c_loc or query in skills_str:
                follower_count = len(c.followers)
                is_following = False
                if user_id:
                    is_following = any(f.user_id == user_id for f in c.followers)
                ...
                results.append({
                    ...
                    'location': c.location,
                    'website': c.website,
                    'description': c.description[:150] + '...' if c.description and len(c.description) > 150 else (c.description or ''),
                    'open_jobs_count': len([j for j in c.jobs if j.status == 'Open']),
                    'open_internships_count': len([i for i in c.internships if i.status == 'Open']),
                    'skills': list(skills),
